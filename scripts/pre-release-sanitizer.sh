#!/bin/bash
# Pre-Release Sanitizer for DigitalEnv
...
print_header() {
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE}  DigitalEnv Pre-Release Sanitizer${NC}"
    echo -e "${BLUE}================================${NC}"
    echo
}
    ...
    for file in "${files_to_fix[@]}"; do
        if [[ -f "$PROJECT_ROOT/$file" ]]; then
            if [[ "$DRY_RUN" == true ]]; then
                ...
                sed -i '' 's|/Users/[^/]*/Dropbox/Admin/DigitalEnv/|./|g' "$PROJECT_ROOT/$file"
...
---
*Generated by DigitalEnv Pre-Release Sanitizer*
EOF
